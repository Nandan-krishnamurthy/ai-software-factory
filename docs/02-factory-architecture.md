...
| IDLE_AT_GATE_C | No story is in progress or in review; at least one story is unblocked, by the same rule `pick` applies (requirements §4: `status:ready` and every "Blocked by" issue closed). If open stories remain but none is unblocked, the state is NEEDS_HUMAN. | **Report "say continue"** | Run S6 → S11 |
